...
#define FLT_MAX 3.40282347e+38
        ...
        #define _BLOCK_SIZE RD_WG_SIZE_0_0
#elif defined(RD_WG_SIZE_0)
        #define _BLOCK_SIZE RD_WG_SIZE_0
#elif defined(RD_WG_SIZE)
        #define _BLOCK_SIZE RD_WG_SIZE
#else
        #define _BLOCK_SIZE 256
#endif

#ifdef RD_WG_SIZE_1_0
     #define _BLOCK_SIZE2 RD_WG_SIZE_1_0
#elif defined(RD_WG_SIZE_1)
     #define _BLOCK_SIZE2 RD_WG_SIZE_1
#elif defined(RD_WG_SIZE)
     #define _BLOCK_SIZE2 RD_WG_SIZE
#else
     #define _BLOCK_SIZE2 256
#endif


// hard-coded for AGC server for now.

int BLOCK_SIZE;
int BLOCK_SIZE2;

#ifdef USE_GPU
#define _DEVICE_TYPE_IDX CL_DEVICE_TYPE_GPU
#define _TARGET_PLATFORM 1
#else
#define _DEVICE_TYPE_IDX CL_DEVICE_TYPE_CPU
#define _TARGET_PLATFORM 0
#endif
// ---
...
*/
	...
	cl_int errnum = clGetPlatformIDs(1, NULL, &numPlatforms);

	// create OpenCL context
	cl_platform_id platform_id[numPlatforms];
	if (clGetPlatformIDs(numPlatforms, platform_id, NULL) != CL_SUCCESS) { printf("ERROR: clGetPlatformIDs(1,*,0) failed\n"); return -1; }
	cl_context_properties ctxprop[] = { CL_CONTEXT_PLATFORM, (cl_context_properties)platform_id[_TARGET_PLATFORM], 0};
	device_type = use_gpu ? CL_DEVICE_TYPE_GPU : CL_DEVICE_TYPE_CPU;
	context = clCreateContextFromType( ctxprop, device_type, NULL, NULL, NULL );
	if( !context ) { printf("ERROR: clCreateContextFromType(%s) failed\n", use_gpu ? "GPU" : "CPU"); return -1; }
	...
	size_t value_size;
	clGetDeviceInfo(device_list[_TARGET_PLATFORM], CL_DEVICE_NAME, 0, NULL, &value_size);
	char *info_dev_name = (char*)malloc(value_size);
	clGetDeviceInfo(device_list[_TARGET_PLATFORM], CL_DEVICE_NAME, value_size, info_dev_name, NULL);
	printf("Device: %s\n", info_dev_name);
	free(info_dev_name);
...
{
#ifdef USE_GPU
	char *rv = getenv("KMEANS_BLOCK_SIZE");
	if (rv != NULL) {
		BLOCK_SIZE = atoi(rv);
	}
	rv = getenv("KMEANS_BLOCK_SIZE2");
	if (rv != NULL) {
		BLOCK_SIZE2 = atoi(rv);
	}
#else
	BLOCK_SIZE = _BLOCK_SIZE;
	BLOCK_SIZE2 = _BLOCK_SIZE2;
#endif
	printf("kmeans initialize BLOCK_SIZE: %d\n", BLOCK_SIZE);
	printf("kmeans initialize BLOCK_SIZE2: %d\n", BLOCK_SIZE2);
