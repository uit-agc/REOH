...
main ( int argc, char *argv[] )
{
	int BLOCK_SIZE;
#ifdef USE_GPU
		char *rv = getenv("LUD_BLOCK_SIZE");
		if (rv != NULL) {
			BLOCK_SIZE = atoi(rv);
			printf("lud initialize BLOCK_SIZE: %d\n", BLOCK_SIZE);
		}
#else
  ...
  printf("WG size of kernel = %d X %d\n", BLOCK_SIZE, BLOCK_SIZE);
	...
	for (i=0; i < matrix_dim-BLOCK_SIZE; i += BLOCK_SIZE) {
	  ...
	  clSetKernelArg(diagnal, 4, sizeof(cl_int), (void*) &BLOCK_SIZE);
	  ...
	  clSetKernelArg(perimeter, 6, sizeof(cl_int), (void*) &BLOCK_SIZE);
	  ...
	  clSetKernelArg(internal, 5, sizeof(cl_int), (void*) &BLOCK_SIZE);
