...
{
#ifdef USE_GPU
	char *rv = getenv("BP_THREADS");
	if (rv != NULL) {
		THREADS = atoi(rv);
		printf("BP initialize # THREADS: %d\n", THREADS);
	}
#endif
...
int bpnn_train_kernel(BPNN *net, float *eo, float *eh)
{
	...
	clSetKernelArg(kernel1, 8, sizeof(cl_int), (void*) &THREADS);
	...
	clSetKernelArg(kernel2, 6, sizeof(cl_int), (void*) &THREADS);
