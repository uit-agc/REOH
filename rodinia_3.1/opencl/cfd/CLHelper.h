...
void _clInit()throw(string){
    ...
    // Select the target platform. Default: first platform 
    targetPlatform = allPlatforms[_TARGET_PLATFORM];
...
*/
	oclHandles.cl_status = clGetDeviceIDs(targetPlatform, _DEVICE_TYPE_IDX, 0, NULL, &deviceListSize);
	if(oclHandles.cl_status!=CL_SUCCESS){
		throw(string("exception in _clInit -> clGetDeviceIDs"));   	
	}
	if (deviceListSize == 0) {
		throw(string("InitCL()::Error: No device found."));
	}
	...
	oclHandles.cl_status = clGetDeviceIDs(targetPlatform, _DEVICE_TYPE_IDX, deviceListSize, oclHandles.devices, NULL);
	if (oclHandles.cl_status != CL_SUCCESS) {
	...
	// print used_device
	size_t value_size;
	clGetDeviceInfo(oclHandles.devices[_TARGET_PLATFORM], CL_DEVICE_NAME, 0, NULL, &value_size);
	char *info_dev_name = (char*)malloc(value_size);
	clGetDeviceInfo(oclHandles.devices[_TARGET_PLATFORM], CL_DEVICE_NAME, value_size, info_dev_name, NULL);
	printf("Device: %s\n", info_dev_name);
	free(info_dev_name);
