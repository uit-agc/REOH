...
int main(int argc, char * argv[])
{
	...
	try{
		...
		int env_max_threads_per_block;
#ifdef USE_GPU
		char *rv = getenv("BFS_MAX_THREADS_PER_BLOCK");
		if (rv != NULL) {
			env_max_threads_per_block = atoi(rv);
			printf("bfs initialize: %d max_threads_per_block\n", env_max_threads_per_block);
		}
#else
		env_max_threads_per_block = MAX_THREADS_PER_BLOCK;		
#endif

		//Make execution Parameters according to the number of nodes
		//Distribute threads across multiple Blocks if necessary
		if(no_of_nodes>env_max_threads_per_block){
			num_of_blocks = (int)ceil(no_of_nodes/(double)env_max_threads_per_block); 
			num_of_threads_per_block = env_max_threads_per_block; 
		}
